...
// algorithm kmp_search wikipedia and book :: http://en.wikipedia.org/wiki/Knuth%E2%80%93Morris%E2%80%93Pratt_algorithm#KMP_algorithm
int kmpSearch(int currentSize){
    int m = 0, i = 0;
    while((m + i) < currentSize){
        if(P[i] == T[m + i]){
            if(i == (currentSize - 1)){
                return m;
            }
...
int main(void){
    ...
    while(cin >> T){
        ...
        */
        memset(TA, -1, sizeof(char) * MAX_N);
        memset(P, -1, sizeof(char) * MAX_N);
        ...
        int appendAmount = kmpSearch(currentSize);
        /*
