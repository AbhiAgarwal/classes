object hw07 extends js.util.JsApp {
  ...
  // That eliminates consecutive duplicates of list elements.
  ...
  // Re implement the compress function from the previous part as compressFold using the foldRight method from the List class in the standard library.
  // The call to foldRight has been provided for you. 
  // Do not call compressFold recursively or any other List methods.
  ...
  // that finds the first element in l where f applied to it returns a Some(a) for some value a.
  // It should replace that element with a and leave l the same everywhere else.
  ...
  sealed abstract class Tree {
    ...
    // Here, we have implemented the binary search tree operation insert as a method of Tree.
    // For this exercise, complete the higher-order method foldLeft.
    // This method performs an in-order traversal of the input tree this calling the callback f to accumulate a result.
    // Suppose the in-order traversal of the input tree yields the following sequence of data values: d1, d2, ..., dn.
